"""PancakeSwap adapter with pool and swap helpers."""
...
class PancakeSwapAdapter(ExchangeAdapter):
    websocket_url = "wss://bsc-ws-node.nariox.org:443"
    rest_base_url = "https://api.pancakeswap.info/api/v2"

    def __init__(self, *, web3_client: Any | None = None, pools: dict[str, dict[str, float]] | None = None) -> None:
        super().__init__("pancakeswap", rate_limit_per_second=5)
    ...
    async def place_order(self, order: dict[str, Any]) -> dict[str, Any]:
        pool = self._pools.get(order["symbol"], {"mid": 0.0, "slippage_bps": 30.0})
        slippage_bps = float(pool.get("slippage_bps", 30.0))
